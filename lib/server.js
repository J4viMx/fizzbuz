...
app.get("/v1/explorers/amount/:mission", (request, response) =>{
    ...
    response.json({mission: request.params.mission,
...
app.get("/v1/explorers/usernames/:mission", (request, response) =>{
    ...
    response.json({mission: request.params.mission,
        explorers: explorersUsernamesByMission});
});

app.get("/v1/fizzbuz/:number", (request, response) =>{
    const number = request.params.number;
    const getApplyValidationInNumber = ExplorerController.getApplyValidationInNumber(number);
    response.json({score: request.params.number,
        tick: getApplyValidationInNumber});
});